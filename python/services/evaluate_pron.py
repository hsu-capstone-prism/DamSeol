...
def evaluate_pronunciation(text_original, text_input):
    ...
    2. 발음 평가
        - 발음 기호로 변환된 사용자 발화와 발음 기호로 변환된 정답 텍스트를 비교하여 평가
        - 동일하거나 유사 음운으로 실현 가능한 경우 정답으로 간주
        - 발음이 빠르거나 단어 간 경계가 불명확할 경우, 속도 조절과 또박또박한 발음을 위한 조언 제공

    2.1 반복 텍스트 처리
        - '있있어요', '같같이', '하세요요'와 같이 어색하게 반복된 음절은 반복 오류로 간주하고 틀린 것으로 평가하지 마십시오
        - 반복된 음절은 1회로 간주하여 평가하며, 피드백에서 언급하지 마십시오
        - 아래 예시와 같은 평가는 해선 안됩니다.
            예: "'있있어요'에서 '있'이 반복되어 어색했어요."

    3. 최종 평가
        - 마지막 출력에서 사용자의 발음을 인용할 때에는 정답 텍스트 기준으로 평가 결과를 구성하십시오
