    ...
    def load(self, path):
       ...
       """
        ...
        if torch.cuda.is_available():
            resume_checkpoint = torch.load(os.path.join(self.BASE_PATH, "outputs", path, self.TRAINER_STATE_NAME), weights_only=False)
            model = torch.load(os.path.join(self.BASE_PATH, "outputs", path, self.MODEL_NAME), weights_only=False)
